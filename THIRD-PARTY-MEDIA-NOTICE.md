...
0.5.0 的公开安装包和更新包不包含来源或再分发权尚未核实的第三方舰船图片和星系地图。
对应页面可能显示应用自带占位图，不影响账号、舰队、房间、通讯和浮层的主要功能。

每个正式载荷必须包含通过状态的 `THIRD-PARTY-MEDIA-AUDIT.json`。对 0.5.0，该报告必须记录：
...
system maps. StarBridge 0.5.0 public installers and update archives also omit
...
Every official 0.5.0 payload must include a passing
