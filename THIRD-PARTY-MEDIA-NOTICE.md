...
当前公开测试版为 0.7.0.2。以下区分公开源码的素材边界与历史发行包的审计记录；
素材不会因为随客户端分发而自动获得开源许可。
...
0.6.6.1 的公开安装包和更新包不包含来源或再分发权尚未核实的第三方舰船图片和星系地图。
对应页面可能显示应用自带占位图，不影响账号、舰队、房间、通讯和浮层的主要功能。

每个正式载荷必须包含通过状态的 `THIRD-PARTY-MEDIA-AUDIT.json`。对 0.6.6.1，该报告必须记录：
