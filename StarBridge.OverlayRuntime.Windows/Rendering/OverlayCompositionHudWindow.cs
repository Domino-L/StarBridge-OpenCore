...
internal sealed partial class OverlayCompositionHudWindow : IOverlayHost, IDisposable
{
    ...
    private void RenderThreadMain()
    {
        ...
        catch (Exception exception)
        {
            ...
            DesktopRuntimeDiagnostics.WriteCrashLog(exception);
            DisposeOnRenderThread();
        }
    }
        ...
        try
        {
            ...
            if (!_glowDevicePathDisabled && _d2dDeviceContext is not null)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    DesktopRuntimeDiagnostics.WriteDiagnosticLog($"overlay-d2d-device-context fallback=legacy error={exception.GetType().Name} message={exception.Message}");
                }
            }
    ...
    private static void LogPerformance(string operation, Stopwatch stopwatch, bool force = false)
    {
        ...
        DesktopRuntimeDiagnostics.WriteDiagnosticLog($"overlay-perf operation={operation} elapsedMs={stopwatch.Elapsed.TotalMilliseconds:F1}");
    }
