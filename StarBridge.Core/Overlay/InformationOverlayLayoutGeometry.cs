...
public readonly record struct InformationOverlayRect(
    double Left,
    double Top,
    double Width,
    double Height)
...
/// <summary>
/// Resolves persisted information-overlay layout values into effective screen
/// rectangles. Editors and Windows renderers should treat this module as the
/// authority for anchors, module limits and responsive collision correction.
/// </summary>
public static class InformationOverlayLayoutGeometry
{
    public const double EventNotificationEdgeInset = 0;
    public const double EventNotificationVerticalInset = 0;

    public static InformationOverlayRect ResolveItemRect(
        InformationOverlayLayoutItem item,
        double surfaceWidth,
        double surfaceHeight)
    {
        ...
        return new InformationOverlayRect(
    ...
    /// <summary>
    /// Removes only collisions introduced by per-module minimum sizes.
    /// Deliberate overlap in persisted normalized geometry remains untouched.
    /// The input items are never mutated.
    /// </summary>
    public static IReadOnlyDictionary<string, InformationOverlayRect> ResolveItems(
        IEnumerable<InformationOverlayLayoutItem> items,
        double surfaceWidth,
        double surfaceHeight)
    {
    ...
    public static InformationOverlayRect ResolveEventNotificationRect(
        double surfaceWidth,
        double surfaceHeight,
        ...
        double snapSize = 0)
    {
        ...
        return new InformationOverlayRect(left, top, width, height);
    }

    public static double ResolveEventNotificationWidth(double surfaceWidth) =>
        Math.Clamp(Math.Max(1, surfaceWidth) * 0.22, 320, 380);

    private static double ResolveLeft(
        InformationOverlayLayoutItem item,
    ...
    private static double ResolveTop(
        InformationOverlayLayoutItem item,
    ...
    private static OverlayModuleConstraints GetModuleConstraints(
        string key,
        double surfaceWidth,
        double surfaceHeight)
    {
        ...
        return key switch
        {
            ...
            "Squads" => new OverlayModuleConstraints(240, isWide ? 460 : 540, 150, 620),
    ...
    private static InformationOverlayRect ResolveIntendedRect(
        InformationOverlayLayoutItem item,
        double surfaceWidth,
        double surfaceHeight)
    {
    ...
    private static double HorizontalOverlapRatio(
        InformationOverlayRect first,
        InformationOverlayRect second)
    {
