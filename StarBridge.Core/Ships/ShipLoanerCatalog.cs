...
public sealed partial class ShipLoanerCatalog
{
    private const string ConceptDisplayRule = "OnlyWhenSourceIsConcept";

    private readonly IReadOnlyDictionary<string, IReadOnlyList<ShipLoanerMatrixEntry>> _entries;
    public bool Available { get; }
    public ShipLoanerCatalog(string? tsv)
    {
        Available = !string.IsNullOrWhiteSpace(tsv) && tsv.Length <= 1024 * 1024
            && tsv.TrimStart('\uFEFF').Split('\n')
                .FirstOrDefault(line => !string.IsNullOrWhiteSpace(line) && !line.StartsWith('#'))
                ?.StartsWith("源英文飞船名\t", StringComparison.Ordinal) == true;
        _entries = BuildEntriesBySource(Available ? tsv : null);
    }
    ...
    public IReadOnlyList<ShipLoanerMatrixEntry> FindForConceptSource(string? sourceEnglishName)
    {
        var key = NormalizeKey(sourceEnglishName);
        if (string.IsNullOrWhiteSpace(key) ||
            !_entries.TryGetValue(key, out var entries))
        {
            return [];
        }
    ...
    public static T? FindLoanerCatalog<T>(string loanerEnglishName, Func<string, T?> find) where T : class
    {
        foreach (var candidate in BuildLoanerLookupCandidates(loanerEnglishName))
        {
            var entry = find(candidate);
    ...
    private static IReadOnlyDictionary<string, IReadOnlyList<ShipLoanerMatrixEntry>> BuildEntriesBySource(string? tsv)
    {
        if (tsv is null)
        {
