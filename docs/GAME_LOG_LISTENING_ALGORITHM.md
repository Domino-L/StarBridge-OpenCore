...
### Flutter Native Host 补充（2026-09-12）

`GameLogIdentityReader` 按完整换行边界增量读取，并严格校验 UTF-8。
真实游戏诊断行可能夹杂非 UTF-8 字节：损坏行不作为身份、服务器、舰船或地点证据，
也不应将整份可读文件归为不可读、反复从头重放。其他完整有效行继续处理，
无需用户重新选择或刷新。若损坏行仍含身份标记 `nickname=` 或退出标记 `SystemQuit`，
当前身份证据保守失效，直到新文件/新进程会话重新建立；不以替换字符猜测身份。
路径/权限/文件句柄安全校验、当前会话时间边界和既有具体规则优先顺序保持不变。
