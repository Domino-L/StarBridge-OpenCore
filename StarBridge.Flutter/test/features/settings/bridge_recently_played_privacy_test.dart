...
import 'package:starbridge_flutter/features/settings/bridge_recently_played_privacy.dart';
...
void main() {
  test('reads the default and writes the revisioned account setting', () async {
    final harness = Harness();
    addTearDown(harness.close);

    final initial = await harness.adapter.read();
    expect(initial.enabled, isTrue);
    expect(initial.enabledAt, isNull);
    expect(initial.disabledAt, isNull);
    expect(initial.revision, 0);

    final saved = await harness.adapter.save(false, 0);
    expect(saved.outcome, RecentlyPlayedPrivacyWriteOutcome.completed);
    expect(saved.snapshot?.enabled, isFalse);
    expect(saved.snapshot?.revision, 1);
    final write = harness.requests.singleWhere(
      (request) => request.name == 'recentlyPlayed.privacyWrite',
    );
  ...
  test(
    ...
    () async {
      final harness = Harness(
        writeError: 'recentlyPlayed.privacy_outcome_unknown',
        applyUnknownWrite: true,
      );
      addTearDown(harness.close);
      final module = RecentlyPlayedPrivacyModule(harness.adapter);
      addTearDown(module.dispose);
      await module.initialize();

      expect(await module.save(false), isTrue);
      expect(module.projection.value.snapshot.enabled, isFalse);
      expect(module.projection.value.snapshot.revision, 1);
  ...
  test('unconfirmed write keeps the last authoritative value', () async {
    ...
    expect(module.projection.value.snapshot.enabled, isTrue);
    expect(module.projection.value.snapshot.revision, 0);
    expect(
  ...
  test('revision conflict preserves the last authoritative value', () async {
    final harness = Harness(writeError: 'recentlyPlayed.write_conflict');
    addTearDown(harness.close);
    final module = RecentlyPlayedPrivacyModule(harness.adapter);
    addTearDown(module.dispose);
    await module.initialize();
  ...
  test('account change cancels a pending confirmation readback', () async {
    ...
    final pending = harness.adapter.save(false, 0);
    ...
    expect(result.outcome, RecentlyPlayedPrivacyWriteOutcome.rejected);
    expect(result.failure, RecentlyPlayedPrivacyFailure.identityUnavailable);
    expect(harness.privacyWrites, 1);
  });
}
...
final class Harness {
  Harness({
    ...
    adapter = BridgeRecentlyPlayedPrivacy(
      session,
  ...
  late final BridgeRecentlyPlayedPrivacy adapter;
  late final StreamSubscription<BridgeEnvelope> subscription;

  int get privacyReads => requests
      .where((request) => request.name == 'recentlyPlayed.privacyRead')
      .length;

  int get privacyWrites => requests
      .where((request) => request.name == 'recentlyPlayed.privacyWrite')
      .length;

  Future<void> _reply(BridgeEnvelope request) {
    if (request.name == 'recentlyPlayed.privacyWrite' &&
        (writeError == null || applyUnknownWrite)) {
      serverValue = request.payload['enabled']! as bool;
      revision++;
      if (serverValue) {
    ...
    return connection.send(
      BridgeEnvelope(
        ...
        status:
            request.name == 'recentlyPlayed.privacyWrite' && writeError != null
            ? 'error'
            : 'ok',
        error:
            request.name == 'recentlyPlayed.privacyWrite' && writeError != null
